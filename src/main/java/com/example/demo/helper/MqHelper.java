...
import com.example.demo.constants.MqExchangeContants;
...
public class MqHelper {
    ...
    public boolean convertAndSend(MqConfig config) {
        Preconditions.checkNotNull(config.getQueue(), "queue is null");
        Preconditions.checkNotNull(config.getMsg(), "msg is null");
...
//        rabbitTemplate.convertAndSend(MqExchangeContants.DELAY_EXCHANGE, config.getQueue().getValue(), json, messagePostProcessor);
        rabbitTemplate.convertAndSend(config.getQueue().getValue(), json);
        return true;
    }
